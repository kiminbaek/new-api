...
func (p *GitHubProvider) GetUserInfo(ctx context.Context, token *OAuthToken) (*OAuthUser, error) {
	...
	if res.StatusCode != http.StatusOK {
		if _, err := readOAuthResponseBody(res.Body); err != nil {
			logger.LogError(ctx, fmt.Sprintf("[OAuth-GitHub] GetUserInfo failed: status=%d, invalid response size", res.StatusCode))
		} else {
	...
	logger.LogDebug(ctx, "[OAuth-GitHub] GetUserInfo succeeded")
