...
func UniversalVerify(c *gin.Context) {
	...
	case secureVerificationMethodPassword:
		...
		if common.PasswordLoginEncryptionEnabled {
			if request.PasswordEncrypted == "" || request.EncryptionKeyID == "" {
		...
		valid, err := model.VerifyUserPassword(identity.UserID, password)
		if err != nil {
			common.ApiError(c, err)
			return
		}
...
func isAllowedSecurityProofScope(scope string) bool {
	switch scope {
	case securityProofScopeChannelKeyRead, securityProofScopePasskeyRegister, securityProofScopePasskeyDelete, securityProofScopePATRotate:
		return true
	default:
		return false
	}
