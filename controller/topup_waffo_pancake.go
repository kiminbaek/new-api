...
func WaffoPancakeWebhook(c *gin.Context) {
	...
	if isSubscription {
		...
		if err := model.CompleteSubscriptionOrder(tradeNo, string(bodyBytes), model.PaymentProviderWaffoPancake, "", model.SubscriptionPaymentConfirmation{Amount: event.Data.Amount, Currency: event.Data.Currency, Product: event.Data.ProductName}); err != nil {
