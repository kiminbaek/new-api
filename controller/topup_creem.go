...
func handleCheckoutCompleted(c *gin.Context, event *CreemWebhookEvent) {
	...
	if err := model.CompleteSubscriptionOrder(referenceId, common.GetJsonString(event), model.PaymentProviderCreem, "", model.SubscriptionPaymentConfirmation{Amount: decimal.NewFromInt(int64(event.Object.Order.AmountPaid)).Shift(-2).String(), Currency: event.Object.Order.Currency, Product: event.Object.Product.Id}); err == nil {
